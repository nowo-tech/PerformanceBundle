...
2. **Product behavior** — what **PerformanceBundle** guarantees to applications that integrate it (see [`USAGE.md`](USAGE.md), [`CONFIGURATION.md`](CONFIGURATION.md), [`INSTALLATION.md`](INSTALLATION.md)). **PHPUnit** and **PHPStan** enforce contracts in CI where applicable.
...
| ID | Story |
| --- | --- |
| US-01 | **As a** performance engineer, **I want** per-route timing metrics **so that** I spot slow controllers in production-like data. |
| US-02 | **As an** operator, **I want** DB query count and duration captured **so that** N+1 issues surface alongside wall time. |
| US-03 | **As an** integrator, **I want** configurable collectors and thresholds **so that** I tune noise vs signal. |
| US-04 | **As a** maintainer, **I want** PHPUnit and PHPStan in CI **so that** metric aggregation logic stays stable. |
...
**Goal:** Symfony bundle for tracking and analyzing route performance metrics. Records request time, database queries count, and query execution time for performance analysis.
