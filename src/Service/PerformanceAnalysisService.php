...
class PerformanceAnalysisService
{
     ...
     *
     * @param array<RouteDataWithAggregates> $routes Array of route data with aggregates
     *
    ...
    private function calculateCorrelation(array $x, array $y): ?array
    {
        ...
        $sumXY = array_sum(array_map(static fn (array $p): float => $p['x'] * $p['y'], $pairs));
        $sumX2 = array_sum(array_map(static fn (array $p): float => $p['x'] * $p['x'], $pairs));
        $sumY2 = array_sum(array_map(static fn (array $p): float => $p['y'] * $p['y'], $pairs));
    ...
    public function analyzeEfficiency(array $routes): array
    {
        ...
        $avgQueryRatio = $queryRatios === [] ? null : array_sum($queryRatios) / count($queryRatios);

        // Sort by severity
        usort($queryBottleneckRoutes, static fn (array $a, array $b): int => $b['query_ratio'] <=> $a['query_ratio']);
        usort($inefficientRoutes, static fn (array $a, array $b): int => $b['request_time'] <=> $a['request_time']);
     ...
     *
     * @return list<array<string, string>> Recommendations
     */
     ...
     *
     * @param array<RouteDataWithAggregates> $routes Array of route data with aggregates (must have getAccessCount())
     *
    ...
    public function analyzeTrafficDistribution(array $routes): array
    {
        ...
        $totalAccesses = array_sum(array_map(static fn (RouteDataWithAggregates $r): int => $r->getAccessCount(), $routes));
        $sortedRoutes  = $routes;
        usort($sortedRoutes, static fn (RouteDataWithAggregates $a, RouteDataWithAggregates $b): int => $b->getAccessCount() <=> $a->getAccessCount());
        ...
        $hotPathAccesses      = array_sum(array_map(static fn (RouteDataWithAggregates $r): int => $r->getAccessCount(), $hotPaths));
        $trafficConcentration = $totalAccesses > 0 ? ($hotPathAccesses / $totalAccesses) * 100 : null;

        return [
            'total_accesses' => $totalAccesses,
            'hot_paths'      => array_map(static fn (RouteDataWithAggregates $r): array => [
                'route'        => $r,
                'access_count' => $r->getAccessCount(),
                'percentage'   => $totalAccesses > 0 ? round(($r->getAccessCount() / $totalAccesses) * 100, 2) : 0,
            ], $hotPaths),
            'cold_paths' => array_map(static fn (RouteDataWithAggregates $r): array => [
                'route'        => $r,
                'access_count' => $r->getAccessCount(),
            ], $coldPaths),
