 ...
 *
 * @method string trans(string $id, array<string, mixed> $parameters = [], string|null $domain = null, string|null $locale = null)
 *
...
class PerformanceController extends AbstractController
{
     ...
     * @param array<string> $requiredRoles Required roles to access the dashboard
     */
    ...
    /**
     * Normalize env value from request/form to string for type-safe use in models and services.
     */
    private function normalizeEnv(mixed $value): string
    {
        if (is_string($value)) {
            return $value;
        }
    ...
    public function index(Request $request): Response
    {
        ...
        $env       = $this->normalizeEnv($formData['env'] ?? $this->getParameter('kernel.environment'));
        ...
        if ($path !== null && $path !== '') {
            $filters['route_path_pattern'] = $path;
        }
        $minRequestTime = $formData['min_request_time'] ?? null;
        if ($minRequestTime !== null) {
            $filters['min_request_time'] = (float) $minRequestTime;
        }
        $maxRequestTime = $formData['max_request_time'] ?? null;
        if ($maxRequestTime !== null) {
            $filters['max_request_time'] = (float) $maxRequestTime;
        }
        $minQueryCount = $formData['min_query_count'] ?? null;
        if ($minQueryCount !== null) {
            $filters['min_query_count'] = (int) $minQueryCount;
        }
        $maxQueryCount = $formData['max_query_count'] ?? null;
        if ($maxQueryCount !== null) {
            $filters['max_query_count'] = (int) $maxQueryCount;
        }
        if (($formData['date_from'] ?? null) instanceof DateTimeImmutable) {
            $filters['date_from'] = $formData['date_from'];
        }
        if (($formData['date_to'] ?? null) instanceof DateTimeImmutable) {
        ...
        try {
            ...
            if ($stats === null) {
                if ($filters === [] && ($limit === null || $limit >= 1000)) {
                    $allRoutes = $routes;
                } else {
        ...
        $missingDependencies = [];
        $dependencyStatus    = [];
        if ($this->dependencyChecker instanceof DependencyChecker) {
    ...
    private function calculateStats(array $routes): array
    {
        ...
        $requestTimes = array_values(array_filter(array_map(static fn (RouteDataWithAggregates $r): ?float => $r->getRequestTime(), $routes), static fn ($v): bool => $v !== null));
        $queryTimes   = array_values(array_filter(array_map(static fn (RouteDataWithAggregates $r): ?float => $r->getQueryTime(), $routes), static fn ($v): bool => $v !== null));
        $queryCounts  = array_values(array_filter(array_map(static fn (RouteDataWithAggregates $r): ?int => $r->getTotalQueries(), $routes), static fn ($v): bool => $v !== null));
    ...
    {
        $withCount = array_filter($routes, static fn (RouteDataWithAggregates $r): bool => $r->getAccessCount() > 0);
        usort($withCount, static fn ($a, $b): int => $b->getAccessCount() <=> $a->getAccessCount());

        return array_slice(array_map(static fn (RouteDataWithAggregates $r): array => [
            'name'         => $r->getName() ?? 'N/A',
    ...
    {
        $withTime = array_filter($routes, static fn (RouteDataWithAggregates $r): bool => $r->getRequestTime() !== null && $r->getRequestTime() > 0);
        usort($withTime, static fn ($a, $b): int => ($b->getRequestTime() ?? 0) <=> ($a->getRequestTime() ?? 0));

        return array_slice(array_map(static function (RouteDataWithAggregates $r): array {
            $time = $r->getRequestTime();
    ...
    private function calculateAdvancedStats(array $routes): array
    {
        ...
        $queryTimes   = array_values(array_filter(array_map(static fn (RouteDataWithAggregates $r): ?float => $r->getQueryTime(), $routes), static fn ($v): bool => $v !== null));
        $queryCounts  = array_values(array_filter(array_map(static fn (RouteDataWithAggregates $r): ?int => $r->getTotalQueries(), $routes), static fn ($v): bool => $v !== null));
        $memoryUsages = array_values(array_filter(array_map(static fn (RouteDataWithAggregates $r): float|int|null => $r->getMemoryUsage() ? $r->getMemoryUsage() / 1024 / 1024 : null, $routes), static fn ($v): bool => $v !== null)); // Convert to MB
        $accessCounts = array_values(array_map(static fn (RouteDataWithAggregates $r): int => $r->getAccessCount(), $routes));
     ...
     *
     * @param list<float|int> $values Array of numeric values
    ...
    private function calculateDetailedStats(array $values, string $label, string $unit): array
    {
        ...
        // Median (indices must be int for list access)
        $half   = (int) ($count / 2);
        $median = $count % 2 === 0
        ...
        $rounded     = array_map(static fn ($v): string => (string) round($v, 2), $values);
        $frequencies = array_count_values($rounded);
        arsort($frequencies);
        $mode = (float) key($frequencies);

        // Standard deviation
        $variance = array_sum(array_map(static fn ($v): float|int => ($v - $mean) ** 2, $values)) / $count;
        ...
        $outliers   = array_filter($values, static fn ($v): bool => $v < $lowerBound || $v > $upperBound);
        ...
        if ($max - $min === 0.0 || $max === $min) {
            ...
            $bucketLabels = array_map(static fn (int $i): float => round($min + ($i * $bucketSize), 2), range(0, $buckets - 1));
        }
    ...
    public function statistics(Request $request): Response
    {
        ...
        $envFilter = new StatisticsEnvFilter($this->normalizeEnv($request->query->get('env') ?? $this->getParameter('kernel.environment')));
        ...
        $env = $this->normalizeEnv($envForm->getData()->env ?? $this->getParameter('kernel.environment'));
        ...
        if ($this->analysisService instanceof PerformanceAnalysisService) {
    ...
    private function getRoutesNeedingAttention(array $routes, array $advancedStats): array
    {
        ...
        foreach ($routes as $route) {
            ...
            if ($route->getRequestTime() !== null && isset($requestTimeStats['percentiles'][95]) && $route->getRequestTime() > $requestTimeStats['percentiles'][95]) {
            ...
            if ($route->getTotalQueries() !== null && isset($queryCountStats['percentiles'][95]) && $route->getTotalQueries() > $queryCountStats['percentiles'][95]) {
        ...
        usort($result['slow_request_time'], static fn (array $a, array $b): int => $b['value'] <=> $a['value']);
        usort($result['high_query_count'], static fn (array $a, array $b): int => $b['value'] <=> $a['value']);
        usort($result['high_memory'], static fn (array $a, array $b): int => $b['value'] <=> $a['value']);
     ...
     *
     * @return array<string> Array of environment names
     */
    ...
    public function exportCsv(Request $request): StreamedResponse
    {
        ...
        $response = new StreamedResponse(function () use ($env, $filters): void {
    ...
    public function exportJson(Request $request): Response
    {
        ...
        $data = array_map(static fn (RouteDataWithAggregates $route): array => [
    ...
    public function exportRecordsCsv(Request $request): StreamedResponse
    {
        ...
        $pathParam      = $request->query->get('path');
        $path           = is_string($pathParam) && $pathParam !== '' ? $pathParam : null;
        ...
        $response = new StreamedResponse(static function () use ($records): void {
            $handle = fopen('php://output', 'w');
            if ($handle === false) {
                return;
            }
    ...
    private function buildFiltersFromRequest(Request $request): array
    {
        ...
        if ($request->query->get('path')) {
            $filters['route_path_pattern'] = $request->query->get('path');
        }
    ...
    public function diagnose(Request $request): Response
    {
        ...
        try {
            $connection = $this->metricsService->getConnection();
        ...
        // 4. Table Status Check (only when checkTableStatus is true; otherwise we do not report table issues)
        $tableStatus = [
            ...
            'main_table_checked'      => false,
            'records_table_checked'   => false,
        ];

        if ($this->tableStatusChecker instanceof TableStatusChecker && $this->checkTableStatus) {
            try {
                ...
                $tableStatus['main_table_checked']  = true;
                ...
                if ($recordsStatus !== null) {
                    ...
                    $tableStatus['records_table_checked']   = true;
                }
            } catch (Exception $e) {
        ...
        if ($this->checkTableStatus && $this->enableAccessRecords && $this->recordRepository instanceof \Nowo\PerformanceBundle\Repository\RouteDataRecordRepository && !$this->tableStatusChecker instanceof TableStatusChecker) {
            try {
                ...
                if ($tableStatus['records_table_exists']) {
                    ...
                    $tableStatus['records_table_complete']  = $recordsMissing === [];
                }
            } catch (Exception $e) {
        ...
        try {
            ...
            if ($container instanceof \Psr\Container\ContainerInterface && $container->has('event_dispatcher')) {
                try {
                    ...
                    // Check REQUEST and TERMINATE event listeners (narrow type for PHPStan)
                    if (is_object($eventDispatcher) && method_exists($eventDispatcher, 'getListeners')) {
        ...
        $trackingConditions[] = [
            ...
            'required'  => true,
            'details'   => sprintf('Entorno actual: %s, Permitidos: %s', $this->normalizeEnv($currentEnv), implode(', ', $this->allowedEnvironments)),
        ];
        $trackingConditions[] = [
        ...
        $trackingConditions[] = [
            ...
            'required'  => true,
            'details'   => $mainTableChecked ? null : 'Comprobación desactivada (check_table_status: false)',
        ];
        $trackingConditions[] = [
            'condition' => 'Tabla principal completa (según entidad)',
            'status'    => $mainTableChecked ? $tableStatus['main_table_complete'] : true,
            'required'  => true,
            'details'   => $mainTableChecked
                ? ($tableStatus['main_table_exists'] && !$tableStatus['main_table_complete']
        ...
        if ($this->enableAccessRecords) {
            ...
            $trackingConditions[] = [
                ...
                'required'  => true,
                'details'   => $recordsTableChecked ? null : 'Comprobación desactivada (check_table_status: false)',
            ];
            $trackingConditions[] = [
                'condition' => 'Tabla de registros completa (según entidad)',
                'status'    => $recordsTableChecked ? (bool) ($tableStatus['records_table_complete'] ?? true) : true,
                'required'  => true,
                'details'   => $recordsTableChecked
                    ? ($tableStatus['records_table_exists'] && !($tableStatus['records_table_complete'] ?? true)
        ...
        if (!$diagnostic['environment']['is_allowed']) {
            $issues[] = sprintf('El entorno actual "%s" no está en la lista de entornos permitidos: %s', $this->normalizeEnv($currentEnv), implode(', ', $this->allowedEnvironments));
        }
        ...
        if (!empty($tableStatus['main_table_checked'])) {
        ...
        if (!empty($tableStatus['records_table_checked'])) {
    ...
    public function clear(Request $request): RedirectResponse
    {
        ...
        $env = $this->normalizeEnv($form->getData()->env);
     ...
     * @return RedirectResponse Redirects back to the dashboard
     */
    #[Route(path: '/{id}/delete', name: 'nowo_performance.delete', requirements: ['id' => '\d+'], methods: ['POST'])]
     ...
     * @return RedirectResponse Redirects back to the dashboard
     */
    #[Route(path: '/{id}/review', name: 'nowo_performance.review', requirements: ['id' => '\d+'], methods: ['POST'])]
        ...
        try {
            ...
            if ($this->enableAccessRecords && isset($formData['save_access_records'])) {
                ...
                if ($routeDataToUpdate !== null) {
                    ...
                    $this->metricsService->getEntityManager()->flush();
                }
            }
            ...
            if ($updated) {
                ...
                // Dispatch after event
                if ($this->eventDispatcher instanceof EventDispatcherInterface && $updatedRouteData !== null) {
    ...
    private function getChartData(string $env, ?string $routeName, int $days, string $metric): array
    {
        ...
        $routes = $this->metricsService->getRoutesWithAggregatesFiltered($env, $filters, 'createdAt', 'ASC');
    ...
    public function accessStatistics(Request $request): Response
    {
        ...
        $env        = $this->normalizeEnv($filterData->env ?? $this->getParameter('kernel.environment'));
        $routeName  = $filterData->route;
        if ($routeName === '' || $routeName === null) {
            $routeName = null;
        }
        $path       = $filterData->path !== null && $filterData->path !== '' ? $filterData->path : null;
        ...
        if ($this->recordRepository instanceof \Nowo\PerformanceBundle\Repository\RouteDataRecordRepository) {
            try {
                ...
                $statisticsByMonth     = $this->recordRepository->getStatisticsByMonth($env, $startDate, $endDate, $routeName, $path, $statusCode);
                $heatmapData           = $this->recordRepository->getHeatmapData($env, $startDate, $endDate, $routeName, $path, $statusCode);
                $totalAccessCount      = $this->recordRepository->getTotalAccessCount($env, $startDate, $endDate, $routeName, $path, $statusCode);
            } catch (Exception $e) {
