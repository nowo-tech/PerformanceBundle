...
HELP
    ...
    {
        // DBAL 3.x uses createSchemaManager(); check kept for DBAL 2.x compatibility
    ...
    private function createTableUsingSchemaTool(EntityManagerInterface $entityManager, SymfonyStyle $io): void
    {
        ...
        $sql = $schemaTool->getCreateSchemaSql(array_values($routeDataRecordMetadata));
        ...
        foreach ($sql as $statement) {
            ...
            if ($isMySQL && (preg_match('/CREATE\s+TABLE/i', $statement) && preg_match('/`?id`?\s+INT/i', $statement) && !preg_match('/AUTO_INCREMENT/i', $statement))) {
    ...
    private function updateTableSchema(EntityManagerInterface $entityManager, SymfonyStyle $io, bool $dropObsolete = false): void
    {
        ...
        foreach ($metadata->getFieldNames() as $fieldName) {
            ...
            /** @var mixed $fieldMapping DBAL 2.x returns array, 3.x may return object */
        ...
        $hasChanges = $columnsToAdd !== [] || $columnsToUpdate !== [] || $columnsToDrop !== [];
     ...
     *
     * @param array<string, mixed> $expectedInfo
     */
    private function columnNeedsUpdate(
        ...
        if (strtolower((string) $expectedTypeNormalized) !== strtolower((string) $existingTypeNormalized)) {
            return true;
        }
        ...
        if (is_numeric($expectedDefault) && $existingDefault !== null) {
    ...
    private function getColumnDefinition(array $columnInfo, AbstractPlatform $platform): string
    {
        ...
        if (isset($columnInfo['default'])) {
            ...
            // Skip default for datetime types (defaultValue non-null ensured by isset above)
            if (!in_array($type, ['datetime', 'datetime_immutable', 'date', 'time'], true)) {
    ...
    private function getColumnSQLType(array $columnInfo, AbstractPlatform $platform): string
    {
        ...
        $sqlType = $typeMap[strtolower((string) $type)] ?? 'VARCHAR(255)';
    ...
    private function addMissingIndexes(EntityManagerInterface $entityManager, SymfonyStyle $io, Table $table): void
    {
        ...
        $normalizedExpected = [];
        foreach ($expectedIndexes as $indexName => $raw) {
            /** @var mixed $raw index definition (array or object from attributes) */
            /** @phpstan-ignore function.alreadyNarrowedType, isset.offset (normalization from mixed) */
            $hasColumns = is_array($raw) && isset($raw['columns']);
            /** @phpstan-ignore function.alreadyNarrowedType (normalization from mixed) */
            $columnsPart                    = is_array($raw) ? $raw : [];
            $normalizedExpected[$indexName] = $hasColumns ? $raw : ['columns' => $columnsPart, 'unique' => false];
        }
        ...
        try {
            $reflection = new ReflectionClass(RouteDataRecord::class);
            $attributes = $reflection->getAttributes(UniqueConstraint::class);
            foreach ($attributes as $attribute) {
        ...
        foreach ($table->getIndexes() as $index) {
            $existingIndexes[strtolower(DbalSchemaNameHelper::getLogicalName($index))] = $index;
        }
        ...
        foreach ($normalizedExpected as $indexName => $data) {
            ...
            if (!isset($existingIndexes[strtolower((string) $indexName)])) {
        ...
        foreach ($indexesToAdd as $indexName => $data) {
            $columns       = $data['columns'];
            $isUnique      = $data['unique'];
            $quotedColumns = array_map(fn (string $col): string => $this->quoteIdentifier($platform, $col), $columns);
