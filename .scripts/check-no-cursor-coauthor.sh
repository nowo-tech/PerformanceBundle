...
# Accept a single ref (HEAD, main, sha) or a revision range (before..after).
case "${REF}" in
  *..*)
    if ! git rev-list --max-count=1 "${REF}" >/dev/null 2>&1; then
      echo "ERROR: git revision range not found: ${REF}" >&2
      exit 1
    fi
    ;;
  *)
